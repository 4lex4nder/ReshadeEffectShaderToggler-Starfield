...
{
    if(!SHOWME) discard;
    o = float4(motionToLgbtq(tex2D(Deferred::sMotionVectorsTex, i.uv).xy).rgb, 1);
}

void PSWriteVectors(in VSOUT i, out float2 o : SV_Target0, out float2 p : SV_Target1, out float2 q : SV_Target2)
{
    o = Starfield::get_motion(i.uv);
    p = o;
    
    // normals need to be reoriented
    float3 blah = Starfield::get_normal(i.uv);
    blah.r = 1.0 - blah.r;
    q = Starfield::_encode(blah - 0.5);
}
